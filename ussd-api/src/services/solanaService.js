...
/**
 * Enhanced Solana Service with enterprise-grade security
 * Features:
...
class SolanaService {
 ...
 validatePin(pin) {
   ...
   const isValid = /^[0-9]{4,6}$/.test(pin) && pin.length >= 4 && pin.length <= 6;
   if (!isValid) this.securityStats.validationFailures++;
   return isValid;
 }
 ...
 /**
  * SECURITY: Enhanced crypto purchase calculation
 ...
 async calculateCryptoPurchase(amount) {
   try {
     ...
     const exchangeRate = parseFloat(process.env.DEFAULT_EXCHANGE_RATE) || 1531;
 ...
 async createTransaction(phoneNumber, pin, amount, type = 'crypto') {
   try {
     ...
     const result = {
       ...
       signature: 'mock_signature_' + transactionId.slice(0, 16),
       amount: parseFloat(amount),
       type: type,
       timestamp: Date.now(),
       nonce: nonce,
       explorerUrl: this.getExplorerUrl('mock_signature_' + transactionId.slice(0, 16)),
       securityLevel: 'enhanced',
       version: '2.0.0'
     };
